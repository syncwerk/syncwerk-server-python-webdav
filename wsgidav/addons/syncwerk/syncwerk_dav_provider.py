...
import synserv
from synserv import syncwerk_api, CALC_SHARE_USAGE
from synserv import check_quota as check_repo_quota
from pyrpcsyncwerk import RpcsyncwerkError
from objectstorage import commit_mgr, fs_mgr
from objectstorage.fs import SyncwFile, SyncwDir
from wsgidav.addons.syncwerk.syncwerk_server_utils import SYNCWERK_CONF_DIR, UTF8Dict, utf8_path_join, utf8_wrap
...
# SyncwerkResource
#===============================================================================
class SyncwerkResource(DAVNonCollection):
    def __init__(self, path, repo, rel_path, obj, environ):
        super(SyncwerkResource, self).__init__(path, environ)
        self.repo = repo
        self.rel_path = rel_path
        self.obj = obj
        self.username = environ.get("http_authenticator.username", "")
        self.org_id = environ.get("syncwerk.org_id", "")
        self.is_guest = environ.get("syncwerk.is_guest", False)
...
#        return int(time.time())
    ...
    def getLastModified(self):
        ...
        mtimes = syncwerk_api.get_files_last_modified(self.repo.id, parent, -1)
        for mtime in mtimes:
    ...
    def beginWrite(self, contentType=None, isnewfile=True, contentlength=-1):
        ...
        if syncwerk_api.check_permission_by_path(self.repo.id, self.rel_path, self.username) != "rw":
            raise DAVError(HTTP_FORBIDDEN)
    ...
    def endWrite(self, withErrors, isnewfile=True):
        if not withErrors:
            ...
            if not self.check_repo_owner_quota(isnewfile=isnewfile, contentlength=contentlength):
                raise DAVError(HTTP_FORBIDDEN, "The quota of the repo owner is exceeded")
            syncwerk_api.put_file(self.repo.id, self.tmpfile_path, parent, filename,
                                 self.username, None)
    ...
    def handleDelete(self):
        ...
        parent, filename = os.path.split(self.rel_path)
        syncwerk_api.del_file(self.repo.id, parent, filename, self.username)
    ...
    def handleMove(self, destPath):
        ...
        if syncwerk_api.check_permission_by_path(dest_repo.id, self.rel_path, self.username) != "rw":
            raise DAVError(HTTP_FORBIDDEN)
        ...
        if not syncwerk_api.is_valid_filename(dest_repo.id, dest_file):
            raise DAVError(HTTP_BAD_REQUEST)

        # some clients such as GoodReader requires "overwrite" semantics
        file_id_dest = syncwerk_api.get_file_id_by_path(dest_repo.id, rel_path)
        if file_id_dest != None:
            syncwerk_api.del_file(dest_repo.id, dest_dir, dest_file, self.username)

        syncwerk_api.move_file(self.repo.id, src_dir, src_file,
    ...
    def handleCopy(self, destPath, depthInfinity):
        ...
        syncwerk_api.copy_file(self.repo.id, src_dir, src_file,
...
# SyncwDirResource
#===============================================================================
class SyncwDirResource(DAVCollection):
    def __init__(self, path, repo, rel_path, obj, environ):
        super(SyncwDirResource, self).__init__(path, environ)
        self.repo = repo
        self.rel_path = rel_path
        self.obj = obj
        self.username = environ.get("http_authenticator.username", "")
...
#        return int(time.time())
    ...
    def getMember(self, name):
        ...
        if isinstance(member, SyncwFile):
            return SyncwerkResource(member_path, self.repo, member_rel_path, member, self.environ)
        else:
            return SyncwDirResource(member_path, self.repo, member_rel_path, member, self.environ)
    ...
    def getMemberList(self):
        ...
        if d.version == 0:
            file_mtimes = []
            try:
                file_mtimes = syncwerk_api.get_files_last_modified(self.repo.id, self.rel_path, -1)
            except:
        ...
        for name, dent in d.dirents.iteritems():
            ...
            if dent.is_dir():
                obj = fs_mgr.load_syncwdir(d.store_id, d.version, dent.id)
                res = SyncwDirResource(member_path, self.repo, member_rel_path, obj, self.environ)
            elif dent.is_file():
                obj = fs_mgr.load_syncwerk(d.store_id, d.version, dent.id)
                res = SyncwerkResource(member_path, self.repo, member_rel_path, obj, self.environ)
            else:
                continue
    ...
    def createEmptyResource(self, name):
        ...
        try:
            syncwerk_api.post_empty_file(self.repo.id, self.rel_path, name, self.username)
        ...
        repo = syncwerk_api.get_repo(self.repo.id)
        if not repo:
            raise DAVError(HTTP_INTERNAL_ERROR)
        ...
        if not obj or not isinstance(obj, SyncwFile):
            raise DAVError(HTTP_INTERNAL_ERROR)

        return SyncwerkResource(member_path, repo, member_rel_path, obj, self.environ)
    ...
    def createCollection(self, name):
        ...
        if not syncwerk_api.is_valid_filename(self.repo.id, name):
            raise DAVError(HTTP_BAD_REQUEST)

        syncwerk_api.post_dir(self.repo.id, self.rel_path, name, self.username)
...
#        return int(time.time())
    ...
    def _createRootRes(self, repo, name):
        obj = get_repo_root_syncwdir(repo)
        return SyncwDirResource("/"+name, repo, "", obj, self.environ)
...
# SyncwerkProvider
#===============================================================================
class SyncwerkProvider(DAVProvider):

    def __init__(self, readonly=False):
        super(SyncwerkProvider, self).__init__()
        self.readonly = readonly
        self.tmpdir = "/tmp/syncwerk-webdav"
    ...
    def __repr__(self):
        ...
        if self.readonly:
            rw = "Read-Only"
        return "%s for Syncwerk (%s)" % (self.__class__.__name__, rw)
    ...
    def getResourceInst(self, path, environ):
        ...
        org_id = environ.get("syncwerk.org_id", "")
        is_guest = environ.get("syncwerk.is_guest", False)
        ...
        if isinstance(obj, SyncwDir):
            return SyncwDirResource(path, repo, rel_path, obj, environ)
        return SyncwerkResource(path, repo, rel_path, obj, environ)
...
def resolvePath(path, username, org_id, is_guest):
    ...
    for segment in segments:
        ...
        if not obj or (isinstance(obj, SyncwFile) and i != n_segs-1):
            raise DAVError(HTTP_NOT_FOUND)
...
def get_repo_root_syncwdir(repo):
    root_id = commit_mgr.get_commit_root_id(repo.id, repo.version, repo.head_cmmt_id)
    return fs_mgr.load_syncwdir(repo.store_id, repo.version, root_id)
...
def get_owned_repos(username, org_id):
    if org_id:
        return syncwerk_api.get_org_owned_repo_list(org_id, username)
    else:
        return syncwerk_api.get_owned_repo_list(username)

def get_share_in_repo_list(username, org_id):
    """List share in repos.
    """
    if org_id:
        repo_list = syncwerk_api.get_org_share_in_repo_list(org_id, username,
                                                           -1, -1)
    else:
        repo_list = syncwerk_api.get_share_in_repo_list(username, -1, -1)

    # for repo in repo_list:
    #     repo.user_perm = syncwerk_api.check_repo_access_permission(repo.repo_id,
    #                                                               username)
    return repo_list
...
def get_groups_by_user(username, org_id):
    ...
    """
    if org_id:
        return synserv.get_org_groups_by_user(org_id, username)
    else:
        return synserv.get_personal_groups_by_user(username)
...
def get_group_repos(username, org_id, groups):
    ...
    if org_id:
        ...
        for grp in groups:
            ...
            for r_id in syncwerk_api.get_org_group_repoids(org_id, grp.id):
                # No need to list my own repo
                repo_owner = syncwerk_api.get_org_repo_owner(r_id)
        ...
        for grp in groups:
            # Get group repos, and for each group repos...
            for r_id in syncwerk_api.get_group_repoids(grp.id):
                # No need to list my own repo
                repo_owner = syncwerk_api.get_repo_owner(r_id)
                if repo_owner == username:
                    continue
                group_repos.append(r_id)
    return group_repos
...
def get_repo_last_modify(repo):
    ...
    if repo.head_cmmt_id is not None:
        last_cmmt = synserv.get_commit(repo.id, repo.version, repo.head_cmmt_id)
...
def list_inner_pub_repos(username, org_id, is_guest):
    ...
    if org_id:
        return synserv.list_org_inner_pub_repos(org_id, username)

    return synserv.list_inner_pub_repos(username)
