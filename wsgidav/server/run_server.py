...
:Author: Jonathan Xu, xjqkilling@gmail.com (clean up for Syncwerk's use)
...
Configuration is defined like this:
    ...
    2. Get config file path from SYNCWDAV_CONF environment variable.
...
import ConfigParser
from wsgidav.addons.syncwerk.domain_controller import SyncwerkDomainController
from wsgidav.addons.syncwerk.syncwerk_dav_provider import SyncwerkProvider
...
def _loadSyncwerkSettings(config):
    # Syncwerk cannot support digest auth, since plain text password is needed.
    config['acceptbasic'] = True
    config['acceptdigest'] = False
    config['defaultdigest'] = False

    # Use Syncwerk authenticator
    config['domaincontroller'] = SyncwerkDomainController()

    # Load share_name from syncwdav config file

    # syncwerk
    #   - conf
    #     - syncwdav.conf

    ##### a sample syncwdav.conf, we only care: "share_name"
    # [WEBDAV]
    # enabled = true
    # port = 8090
    # share_name = /syncwdav
    ##### a sample syncwdav.conf

    share_name = '/webdav'

    syncwdav_conf = os.environ.get('SYNCWDAV_CONF')
    if syncwdav_conf and os.path.exists(syncwdav_conf):
        cp = ConfigParser.ConfigParser()
        cp.read(syncwdav_conf)
        section_name = 'WEBDAV'

        if cp.has_option(section_name, 'share_name'):
            share_name = cp.get(section_name, 'share_name')

    # Setup provider mapping for Syncwerk. E.g. /syncwdav -> syncwerk provider.
    provider_mapping = {}
    provider_mapping[share_name] = SyncwerkProvider()
